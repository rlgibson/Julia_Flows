...
for idx_stage in 1:length(get_stages)
    ...
    idx_pre_proppant = findfirst(x->x>=pre_proppant_times[idx_stage], tm_values[idx_stage_raw])
    ...
    mds_unstaggered = [(md_values[idx_stage_raw][i]+md_values[idx_stage_raw][i+1])/2 for i in 1:length(md_values[idx_stage_raw])-1]
    ...
    # put data in combined plot before estimating shift since each stage has different shift - apply xlmits in final plot
    global plots_combined
    if !@isdefined normalize_modes
        normalize_modes = false
    end
    if normalize_modes
        mode_scalar = 1/maximum(measured_signals[idx_stage_raw][:,idx_pre_proppant])
    else
        mode_scalar = 1.0
    end
    plot!(plots_combined,mds_unstaggered, measured_signals[idx_stage_raw][:,idx_pre_proppant].*mode_scalar,label="$(stage_labels[idx_stage])",
          xlabel=dist_label,ylabel="Acoustic energy indicator",
          #xlimits = (xlimits_list[idx_stage][1],xlimits_list[idx_stage][2]),
    ...
    @info "stg=$(stage_labels[idx_stage]), data extrema: $(extrema(measured_signals[idx_stage_raw][:,idx_pre_proppant]))"
    p = plot(mds_unstaggered, measured_signals[idx_stage_raw][:,idx_pre_proppant],label="",color=:gray,
             xlabel=dist_label,ylabel="Acoustic energy indicator",
...
for idx_stage in 1:length(get_stages)
    ...
    plot!(plots_combined,mds_unstaggered, measured_signals[idx_stage_raw][:,idx_post_proppant].*mode_scalar,label="$(stage_labels[idx_stage])",
          xlabel=dist_label,ylabel="Acoustic energy indicator",
          # xlimits = (xlimits_list[idx_stage][1],xlimits_list[idx_stage][2]),
          right_margin=3mm, left_margin=3mm)
